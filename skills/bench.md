...
cargo add --dev tikv-jemalloc-ctl -F stats

1. 设计评测指标

内存占用计算 tikv-jemalloc + 线程的方式

每个测试启动一个线程，在创建数据结构之前记录内存，测试完成之后，数据结构被 drop 之前，记录内存

然后计算差值得到实际内存，用人类可读的方式展示，类似
```
use humansize::{format_size, BINARY};
format_size(size, BINARY)
```

2. 在评测目录 benches/base.rs

定义 trait，所有参与评测的库都实现这个 trait
